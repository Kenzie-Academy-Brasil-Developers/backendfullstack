...
export function paginate( { list, countPerPage, query, filterException }:IPaginate ){
    ...
    if( listFilter.length != 0 ){
        listPaginate = list.filter( item => {
            ...
            listFilter.forEach( search => {
                ...
                const { type:typeValue, result:value } = serializerStringType(search[1])
                const isLevelsKeys = keys.length > 1
    
                if( isLevelsKeys ){
                    const { type:typeKeysTwo, result:keyTwo } = serializerStringType(`${item[keys[0]][keys[1]]}`)
                    ...
                    const { type:typekeyOne, result:keyOne } = serializerStringType(`${item[search[0]]}`)
...
function serializerStringType( query:any ){
    ...
    const isBoolean = query == "true" ? true : query == "false" ? false : undefined
    ...
    const isNumber = !query.includes(".") && Number(query) || !query.includes(".") && Number(query) == 0  ? Number(query) : undefined
